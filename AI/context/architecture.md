...
## Executable Safety Evidence Update

- `benchmarks/cases/policy_cases.json` is the local no-network safety corpus.
- `benchmarks/runner.py` evaluates each case through the real
  `ClosureVerdict -> RewriteDecision` contract and runs selected equivalence
  probes through the subprocess harness.
- `benchmarks/metrics.py` computes total/supported/unsupported, accepted,
  rejected, TP/TN/FP/FN, precision, recall, FPR, FNR, feature coverage, safety
  failures, and equivalence failures.
- `benchmarks/report.py` renders GitHub-readable Markdown with false positive,
  false negative, unsupported, safety, baseline regression, and reproduction
  sections.
- `tools/check_evidence.py` is the CI evidence gate for schema validity,
  `false_positive == 0`, `unsafe_rewrite_count == 0`, and baseline regression.
