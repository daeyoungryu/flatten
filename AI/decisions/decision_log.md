...
## DEC-030 | 2026-06-12 | Verdicts Are Method-Local Authorization

Decision: Group observations by method qualname and require each rewrite call
site to use a SAFE verdict for its own observed method.

Reason: A CLOSED verdict for one method or hierarchy must not authorize a
rewrite for another method. The P0 Worker/Animal regression showed that merging
observations into the first observed qualname can propagate final-class closure
evidence into an unrelated open hierarchy.

Impact: `_verdicts_from_observations()` now emits per-method verdicts, and
`RewritePlanner.plan_from_observations()` selects verdicts per call site instead
of reusing the first allowed verdict globally.

## DEC-031 | 2026-06-12 | Rewrite Validation Requires Explicit Cases

Decision: `rewrite --apply --entry` rejects commands without `--cases`.

Reason: The previous implicit `[((), {})]` verification case gave a false sense
of safety and provided no branch coverage for entry points that accept default
arguments or internally choose dynamic paths.

Impact: CLI integration tests now provide explicit cases, and the P0 regression
test guards against restoring implicit empty-case verification.

## DEC-032 | 2026-06-12 | Soundness Audit Is a Release Artifact

Decision: Add `docs/SOUNDNESS.md` as the canonical audit for the rewrite
pipeline and dynamic-feature classification.

Reason: The project needs a clear, reviewable account of inputs, outputs,
failure modes, conservative refusal rules, and soundness assumptions across
Observation, Closure Analysis, Rewrite Decision, CST Transform, and Validation.

Impact: Future proof artifacts, differential testing, mutation testing, and OSS
benchmark work should refer back to the invariants in `docs/SOUNDNESS.md`.

## DEC-033 | 2026-06-12 | Every Rewrite Plan Carries a Proof Artifact

Decision: Emit a machine-readable `proof_artifact` dictionary on every CLI
rewrite plan.

Reason: A rewrite must be mechanically explainable after the fact. The existing
`RewriteDecision` explains method-level authorization, but each concrete
rewrite plan also needs callsite, observed target, closure status, passed and
failed rules, risk level, and allowed/refused state.

Impact: `flatten plan` JSON now exposes a per-plan proof artifact with callsite,
observed targets, closure status, passed/failed closure rules, risk level, and
authorization state. This becomes the compatibility point for future
`flatten explain`, proof validation gates, and benchmark proof coverage metrics.

## DEC-034 | 2026-06-12 | Mutation Harness Guards False Positives

Decision: Add `flatten.mutations` to generate source-level mutation variants
and treat source-level `setattr` as an UNSAFE monkey-patch risk in CLI planning.

Reason: False-positive prevention needs adversarial variants, not only static
fixtures. New subclasses, new dispatch implementations, monkey patches,
runtime registration, and `setattr` mutations are core ways a previously safe
call site can become unsafe.

Impact: `tests/test_mutation_harness.py` verifies required mutation kinds and
guards against a `setattr` mutation producing a rewrite plan.

## DEC-035 | 2026-06-13 | Benchmark Sanity Is a CI and Release Gate

Decision: Add a 35-project OSS benchmark catalog, `flatten benchmark`, JSON and
Markdown KPI report generation, CI `benchmark-sanity`, and release-gate
benchmark execution.

Reason: Research-grade claims need reproducible benchmark artifacts and
machine-checkable KPI fields before large OSS evaluation starts. The gate must
also prevent regressions in benchmark schema, proof coverage reporting, and
release documentation.

Impact: `benchmarks/projects.csv`, `src/flatten/benchmarks.py`,
`tests/test_benchmarks.py`, `docs/research_evaluation.md`, README quantitative
status, CI, and `scripts/release_gate.ps1` now form the benchmark/release
evidence layer. The current benchmark status is catalog-only with
`Projects Evaluated = 0`; 0.2.0 remains blocked until the external empirical
run evaluates at least 30 pinned OSS projects.

## DEC-036 | 2026-06-13 | Safety Evidence Is an Executable Benchmark Gate

Decision: Add a local executable benchmark suite, baseline comparison, and CI
evidence gate separate from the OSS catalog.

Reason: The OSS catalog is useful for release planning but does not itself
measure rewrite safety. PRs need deterministic, no-network evidence that false
positives and unsafe rewrites remain zero.

Impact: `benchmarks/cases/*.json`, `benchmarks.runner`, benchmark metrics and
